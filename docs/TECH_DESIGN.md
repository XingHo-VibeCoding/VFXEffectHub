...
- **作品数据 = GitHub 永久托管（强持久化）**——写进 `data/works.json` 后 git push，GitHub 帮你存所有版本、历史、备份，只要仓库不被删，数据就在
- **用户行为数据 = 浏览器 localStorage（弱持久化，会丢）**——点赞、收藏、收藏夹仅存在访客自己的电脑上；换设备、清浏览器缓存、隐私模式下都会丢失
- **贴图文件 = 随仓库永久托管（强持久化）**——和作品 JSON 同步走 git
...
| **本项目（用户行为）** | **浏览器 localStorage** | **弱：易失** |
| 纯静态无 localStorage 的页面 | 无任何本地存储 | 不适用——访客一切行为都不留痕 |
